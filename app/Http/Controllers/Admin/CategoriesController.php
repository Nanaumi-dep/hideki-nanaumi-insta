...
class CategoriesController extends Controller
{
    ...
    public function index(){
        $all_categories = $this->category->orderBy('updated_at', 'desc')->paginate(15);

        # This handle the counting of the post that do not have categories.
    ...
    public function store(Request $request){
        ...
        # 2.Save the new category
        $this->category->name = ucwords(strtolower($request->name));
        // DATABASE PROGRAMMING - > database programming - > Database Programming
        $this->category->save();
